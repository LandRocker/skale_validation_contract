...
describe(" P2W Game contract", function () {
  ...
  describe("add planet to game", function () {
    it("should allow adding planet", async function () {
      ...
      const rewardType = wethInstance.address;
      ...
      expect(planet.rewardType).to.equal(wethInstance.address);
  ...
  describe("rover mining", function () {
    ...
    // it.only("should not allow mining if planet dose not staked before", async function () {
    //   await gameInstance
    //     .connect(script)
    //     .addPlanet(
    //       0,
